...
import {
  Users, Plus, Pencil, Trash2, ArrowLeft, X, Eye, EyeOff, ShieldCheck, UserRound, Building2
} from "lucide-react"
import { Button } from "@/components/ui/button"
import Header from "@/components/layout/Header"
import Sidebar from "@/components/layout/Sidebar"
import { getSession, getAccessToken, isMaster } from "@/lib/storage"
...
export default function AdminUsuariosPage() {
  ...
  const [form, setForm] = useState({ empresa_id: "", nome: "", email: "", senha: "", role: "usuario" })
  ...
  useEffect(() => {
    const s = getSession()
    if (!s || !isMaster()) { router.push("/login"); return }
    setSession(s)
    carregarEmpresas()
  }, [router])

  useEffect(() => {
    if (session) carregarUsuarios()
  }, [session, filtroEmpresa])
  ...
  async function carregarUsuarios() {
    setLoading(true)
    const token = getAccessToken()
    const url = filtroEmpresa
      ? `/api/admin/usuarios?empresa_id=${filtroEmpresa}`
      : "/api/admin/usuarios"
  ...
  function abrirCriar() {
    setForm({ empresa_id: filtroEmpresa, nome: "", email: "", senha: "", role: "usuario" })
    setErro(null)
    setShowSenha(false)
    setModal("criar")
  }
  ...
  function abrirEditar(u: Usuario) {
    setEditTarget(u)
    setForm({ empresa_id: u.empresa_id, nome: u.nome, email: u.email, senha: "", role: u.role })
    setErro(null)
    setShowSenha(false)
    setModal("editar")
  }
        ...
        <main className="flex-1 p-6 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-[#64748b]">Todos os usuários de todas as empresas</p>
            </div>
          ...
          {/* Filtro por empresa */}
          <div className="bg-white border border-[#e2e8f0] rounded-xl p-4 mb-5 flex items-center gap-3">
            <Building2 className="w-4 h-4 text-[#64748b]" />
            <select
              value={filtroEmpresa}
              onChange={e => setFiltroEmpresa(e.target.value)}
              className="flex-1 text-sm border border-[#e2e8f0] rounded-lg px-3 py-1.5 text-[#0f172a] focus:outline-none focus:ring-2 focus:ring-[#1a2744]/20"
            >
            ...
            {filtroEmpresa && (
              <button onClick={() => setFiltroEmpresa("")} className="text-[#94a3b8] hover:text-[#64748b]">
                <X className="w-4 h-4" />
              </button>
            )}
          </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-[#f8fafc] border-b border-[#e2e8f0]">
                  <tr>
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-[#64748b] text-xs uppercase tracking-wide">Empresa</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-5 space-y-4">
              {modal === "criar" && (
                <div>
                  <label className="block text-xs font-semibold text-[#64748b] mb-1">Empresa *</label>
                  <select
            ...
            <div className="px-6 pb-5 flex gap-2 justify-end">
              ...
              <Button
                onClick={salvar}
                disabled={salvando || !form.nome || (modal === "criar" && (!form.empresa_id || !form.email || !form.senha))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-[#64748b] mb-5">
              <strong>{confirmDelete.nome}</strong> ({confirmDelete.email}) será removido permanentemente. Esta ação não pode ser desfeita.
            </p>
